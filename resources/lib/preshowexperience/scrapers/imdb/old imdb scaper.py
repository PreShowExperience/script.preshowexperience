...
TRAILERS_URL = BASE_URL + '/title/%s/videogallery/'
...
class Scraper(object):
    ...
    def __get_movies(self, source, limit, details):
        ...
        # Limiting the trailers to 38
        for movie in movies_to_scrape[:38]:
    ...
    def get_trailers(self, location, movie_id):
        try:
            util.DEBUG_LOG('Movie ID: {0}'.format(movie_id))
            url = TRAILERS_URL % (movie_id)
            util.DEBUG_LOG('URL: {0}'.format(url))
