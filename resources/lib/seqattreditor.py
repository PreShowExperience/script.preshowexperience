...
class SeqAttrEditorDialog(kodigui.BaseDialog):
    xmlFile = 'script.preshowexperience-sequence-attribute-editor.xml'
    ...
    def getRating(self, start=None, remove=None, single=False, disp='', prefix=''):
        ratingsList = [r for r in preshowexperience.ratings.defaultRatingsSystem() if start is None or r > start]
        if prefix:
